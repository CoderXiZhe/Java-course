...
<script type="text/javascript">
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data.success){
						...
						$("#remarkDiv").before(html);//在这个div前方追加
						$("#remark").val("");
					}else{
						alert("添加失败")
					}
		...
		$("#updateRemarkBtn").click(function() {
			...
			$.ajax({
				...
				success:function(data){
					 ...
					 */
					if(data.success){
						//更新div中的内容  noteContent editTime editBy
		...
		$.ajax({
			...
			success:function(data){
				...
				$.each(data,function(i,e){
					/*
						javascript:void(0) 超链接禁止使用 只能以触发的方式来操作  onclick
					 */
					...
					html+='<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
					html+='<a class="myHref" href="javascript:void(0);" onclick="updateRemark(\''+e.id+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #FF0000;"></span></a>';
					html+='&nbsp;&nbsp;&nbsp;&nbsp;';
	...
	function updateRemark(id){
		$("#remarkId").val(id);//将模态窗口中的隐藏域中的id进行赋值

		var noteContent =$("#e"+id).html();//找到指定的存放备注信息的标签

		$("#noteContent").val(noteContent);//将展现出来的信息 赋给模态窗口的文本域
